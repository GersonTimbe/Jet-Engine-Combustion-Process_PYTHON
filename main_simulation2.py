...
kT   = 2000                         # thermal conductivity (ajusted)
alpha = kT/(rho*Cp)                 # thermal diffusity

#Constant Velocity field
u_x = 15                            # Maximum Avarage velocity (ajusted)
u_y = 0.0     
...
D_fuelx = 2                       #| Diffusion coefficients
D_Ox    = 2                       #|
D_fuely = 1                       #|
D_Oy    = 1                           #|
...
mfr_fuel = 82/7                      # mass flow rate to the fuel (kg/m2*s - constant VF)             
...
def filling_matrixes_temperature():
    for k in range(Ni):
        i = 1 + k // (Nx-2)
        j = 1 + k % (Nx-2)
...
def filling_matrixes_f(A_f, M_f, T, YO, D_fuelx,D_fuely, u_x):
    for k in range(Ni):
        i = 1 + k // (Nx-2)
        j = 1 + k % (Nx-2)
        #Arrenhius Model
...
def filling_matrixes_O(A_O, M_O,T, Yf, D_Ox,D_Oy, u_x):
    for k in range(Ni):
        i = 1 + k // (Nx-2)
        j = 1 + k % (Nx-2) 
        ...
        COx = -(u_x * Dt) / (2 * delta_x)        
        COy = -(u_y * Dt) / (4 * delta_y)       
        ...
        a_Oij = 1 + 2 * DOx + 2 * DOy - COx +r_ij_O
        b_Oij = 1 - 2 * DOx - 2 * DOy + COy -r_ij_O
...
def apply_boundries_conditions_Yf(Yf):
    for k in range(Ni):
        ...
        if j == 1:
            if i_min < i < i_max:
                Yf[i, 0] = Yf[i, 1] + delta_x*(mfr_fuel/(u_x*rho*Lx))  
            else:
...
def stop_injection_Yf(Yf,step):
    for k in range(Ni):
        ...
        Yf[:,-1] = 0.0  
        Yf[-1,:] = 0.0  
        if j < int(Nx/2)-1 and Yf[i,j] <= 0.00076:
            Yf[i,j] =min(Yf[i,j], 0.00076)
        else:
            None
    return Yf
...
def apply_boundries_conditions_YO(YO):
    for k in range(Ni):
        ...
        if j == 1:
            if i_min < i < i_max:
                YO[i, 0] = YO[i, 1] + delta_x*(mfr_O/(u_x*rho*Lx)) 
            else:
...
def stop_injection_YO(YO, step):
    for k in range(Ni):
        ...
        YO[:,-1] = 0.0  
        YO[-1,:] = 0.0  
        if j < int(Nx/2)-1 and YO[i,j] <= 0.00235:  
            YO[i,j] =min(YO[i,j],0.00235)  
        else:
            None
    return Yf
...
def model_source_term_Q(Yf, YO, Temp):
    ...
    for k in range(Ni):
        i = 1 + k // (Nx - 2)
        j = 1 + k % (Nx - 2)
        q = (1.7*Dt)*Delta_H / (rho*MW_fuel*Cp)
...
for k in range(Ni):
    ...
    if j == 1 and i_min < i < i_max:
        BCs_Term_fuel[i, j] = 2*Sbxf*(delta_x*mfr_fuel/(u_x*rho*Lx))  
    else:
        BCs_Term_fuel[i, j] = 0
...
for k in range(Ni):
    ...
    if j == 1 and i_min < i < i_max:
        BCs_Term_O[i, j] =2*SbxO*(delta_x*mfr_O/(u_x*rho*Lx))
    else:
        BCs_Term_O[i, j] = 0
    ...
    #4.2
        ...
        #4.2.2
            ...
            if step<400:
                ...
                filling_matrixes_f(A_f,M_f,T, YO, D_fuelx, D_fuely, 0)
            ...
            if step<400:
               ...
               filling_matrixes_f(A_O,M_O,T, Yf, D_Ox, D_Oy, 0)
